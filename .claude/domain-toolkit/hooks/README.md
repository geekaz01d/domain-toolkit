# Domain-Toolkit Hooks

Claude Code hooks that automate the domain kit lifecycle.

## session-start.sh

Fires on every Claude Code session start. If the working directory contains `.claude/domain-toolkit/domain.yaml` (the managed domain detection signal), injects context files as opening context:

1. `persona.md` — agent identity, model tier, behavioural settings
...
Non-domain directories (no `domain.yaml`) are unaffected. Global and domain-level governance (`CLAUDE.md`, `AGENTS.md`) is loaded natively by Claude Code.
